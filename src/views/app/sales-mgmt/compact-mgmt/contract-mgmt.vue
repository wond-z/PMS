...
<template>
	<div class="table-component-common">
        ...
        <Card class="margin-bottom-10 margin-top-10 padding-top-bottom-minish">
            <div class="select-col">
                ...
                <div class="div">
                    ...
                    <DatePicker :options="options" type="date" :value='dateRange.start' placement="bottom-end" placeholder="选择日期" @on-change="dateFormatStart"></DatePicker>
                </div>
                <div class="div">
                    <label class="label">截止日期</label>
                    <DatePicker :options="options" type="date" :value='dateRange.end' placement="bottom-end" placeholder="选择日期" @on-change="dateFormatEnd"></DatePicker>
                </div>
        ...
        <Modal class="contract-modal modal-table-form" width='900px' v-model="logModal" :mask-closable="false" >
            ...
            <div>
                <Form ref="postForm" :model="postData" :rules="ruleValidate" label-position="right">                
                    <table class="table-common larger-row table-common-buttom" border="1" cellspacing='0'>
                        ...
                        <tr>
                            <td colspan="4" class="align-left input-left">
                                <FormItem prop="HtDate" label="日期：" inline >
                                    <DatePicker :options="options" type="date" :transfer="true" v-model="postData.HtDate" class="input-left"></DatePicker>
                                </FormItem>
                            </td>
                            <td colspan="5"></td>
                        ...
                        <tr class="leader-td-style">
                            <td colspan="6" >
                                <FormItem prop="Operator" label="经办人：" inline>
                                    <Select placement='top' :transfer="true" v-model="postData.Operator">
                                         <Option v-for="item in adminList" :value="item.name" :key="item.id">{{ item.name }}</Option>
                                    </Select>
                                </FormItem>
                            </td>
                            <td colspan="6">
                                <FormItem prop="DepartmentHead" label="部门主管：" inline>
                                    <Select placement='top' :transfer="true" v-model="postData.DepartmentHead">
                                        <Option v-for="item in adminList" :value="item.name" :key="item.id">{{ item.name }}</Option>
                                    </Select>
                                </FormItem>
                            </td>
        ...
        <!-- 查看界面弹出框 -->
        <Modal class="contract-modal modal-table-form" width='900px' v-model="lookModal" :mask-closable="false" >
            <p slot="header" >
                 <Icon type="compose" size='16'></Icon>
                <span>产品订货合同</span>
            </p>
            <div>
                <table class="table-common larger-row table-common-buttom" border="1" cellspacing='0'>
                    ...
                    <tr>
                        <td colspan="4" class="align-left">日期：{{lookData.zHtDate}} ~ {{lookData.zFinishDate}}</td>
                        <td colspan="5"></td>
                        <td colspan="3" class="align-right">编号：SYC/ZJ-WL-07</td>
                    </tr>
                    <tr>
                        <td rowspan="3">订货单位</td>
                        <td>名称</td>
                        <td colspan="4">{{lookData.bcCp_OrderGdCmp}}</td>
                        <td rowspan="3">收货单位</td>
                        <td>名称</td>
                        <td colspan="4">{{lookData.bcCp_CollectGdCmp}}</td>
                    </tr>
                    <tr>
                        <td>地址</td>
                        <td colspan="4">{{orderAddress}}</td>
                    ...
                    <tr>
                        <td>合同号</td>
                        <td colspan="4">{{lookData.zHt_Code}}</td>
                        <td>运输方式</td>
                        <td colspan="4">{{lookData.zTransportModeName}}</td>
                    </tr>
                    <tr>
                        <td colspan="3">产品名称</td>
                        <td colspan="3">钢级</td>
                        <td colspan="6">规格/螺纹类型</td>
                        <!-- <td colspan="3" class="width-25">螺纹类型</td> -->
                    </tr>
                    <tr>
                        <td colspan="3">{{lookData.bgSubClass_Value}}</td>
                        <td colspan="3">{{lookData.bgSteelGrade}}</td>
                        <td colspan="6">{{lookData.bgSpecifications}}</td>
                        <!-- <td colspan="3">{{lookData.bgThread}}</td> -->
                    </tr>
                    <tr>
                        <td colspan="3" class="width-25"> 长度范围（m）</td>
                        <td colspan="3" class="width-25">订货量（吨）</td>
                        <td colspan="3" class="width-25">订货金额</td>
                        <td colspan="3" class="width-25">交货状态</td>
                    </tr>
                    <tr>
                        <td colspan="3">9.0 ~ 10.0</td>
                        <td colspan="3">{{lookData.zOrderLiang}}</td>
                        <td colspan="3">{{lookData.zOrderAmount}}</td>
                        <td colspan="3">正火</td>
                    </tr>
                    <tr>
                        <td colspan="6">执行标准</td>
                    ...
                    <tr>
                        <td>备注</td>
                        <td colspan="11">{{lookData.zRemarks}}</td>
                    </tr>
                    <tr class="leader-td-style">
                        <td colspan="6" >经办人：{{lookData.zOperator}}</td>
                        <td colspan="6">部门主管：{{lookData.zDepartmentHead}}</td>
                    </tr>
                </table>
            </div>
            <p slot="footer" >
    ...
    data () {
        return {
                ...
                columns: [//首页显示表格的列
                    ...
                    {title: '产品', key: 'bgCpPinMin',align:"center",width:180},
    ...
    methods: {
        ...
        update(data){
            if(data.zIsFinish == 1){
                ...
                this.initData();
                this.lookData = data;
                this.showOrderAddress(data.zCp_OrderGdCmp_ID) 
                this.showRecAddress(data.zCp_CollectGdCmp_ID)           
                this.lookModal = true;
            }
        },
        remove(data){
